...
import { saveProjectIntegrations } from "@/lib/firebase/functions/projectIntegrationsFunctions";
import {
  Select,
  SelectContent,
  SelectItem,
...
export default function ProjectSettingsPage() {
  ...
  useEffect(() => {
    ...
    setGithubRepoUrl(data.integrations?.github?.repoUrl ?? "");
  }, [data]);
  ...
  const handleSaveGithubRepo = async () => {
    ...
    try {
      const result = await saveProjectIntegrations({
        projectId,
        githubRepoUrl: githubRepoUrl || null,
      });

      if (result.ok) {
        toast.success("تم حفظ رابط GitHub", {
          description: "تم تحديث رابط المستودع بنجاح.",
        });
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                ...
                المستودعات والتكاملات
              </CardTitle>
              <CardDescription>
                روابط الريبوهات والخدمات المتصلة
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between p-3 border rounded-lg">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium">Domains & Hosting</p>
                      <p className="text-xs text-muted-foreground">
      ...
      <Dialog open={showGithubDialog} onOpenChange={setShowGithubDialog}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            ...
            <DialogDescription>
              أدخل رابط المستودع على GitHub (مثال: https://github.com/user/repo)
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              <label className="text-sm font-medium">رابط المستودع</label>
              <Input
              ...
              />
              <p className="text-xs text-muted-foreground">
                اترك الحقل فارغاً لإلغاء الربط
              </p>
            </div>
          </div>
          <DialogFooter>
            ...
            <Button onClick={handleSaveGithubRepo} disabled={savingGithub}>
              {savingGithub ? (
                <>
                  <Loader2 className="w-4 h-4 mr-2 animate-spin" />
                  جارٍ الحفظ...
