 ...
 */
export const watchErrorRate = onSchedule({ schedule: 'every 5 minutes', timeZone: 'UTC' }, async (event) => {
console.log('[watchErrorRate] Starting error rate check');
    ...
    try {
      ...
      console.log(
        `[watchErrorRate] Total: ${total}, Errors: ${errors}, Rate: ${(errorRate * 100).toFixed(2)}%`
      );
 ...
 * Runs every 5 minutes
 */
export const watchAuthFails = onSchedule({ schedule: 'every 5 minutes', timeZone: 'UTC' }, async (event) => {
 ...
 */
export const watchQuotaBreach = onSchedule({ schedule: 'every 15 minutes', timeZone: 'UTC' }, async (event) => {
    console.log('[watchQuotaBreach] Starting quota breach check');

    try {
      const thresholdPercent = Number(
        process.env.ALERT_THRESHOLD_QUOTA_PERCENT || 0.95
      );
      ...
      for (const doc of quotasSnapshot.docs) {
        ...
        const tier = (data.planTier || 'free').toLowerCase() as
          | 'free'
          | 'pro'
          | 'enterprise';
        ...
        if (usagePercent >= thresholdPercent) {
          ...
          if (existingAlert.empty) {
            ...
            alertsCreated++;
            console.log(
              `[watchQuotaBreach] Alert created for user ${uid} (${(usagePercent * 100).toFixed(0)}% used)`
            );
          }
        }
      }
 ...
 */
export const watchFunctionErrors = onSchedule({ schedule: 'every 10 minutes', timeZone: 'UTC' }, async (event) => {
